...
KERNEL_FQ void m92000_mxx (KERN_ATTR_VECTOR ())
{
  /**
   * modifier
   */
  ...
  for (u32 il_pos = 0; il_pos < IL_CNT; il_pos += VECT_SIZE)
  {
    ...
    sha1_update_vector (&ctx1, eminor, 8);

    sha1_update_vector (&ctx1, w, pw_len);

    sha1_update_vector (&ctx1, eminorend, 3);
    ...
    sha1_update_vector (&ctx, eminorend, 3);
...
KERNEL_FQ void m92000_sxx (KERN_ATTR_VECTOR ())
{
  /**
   * modifier
   */
